...
/**
 *
 * @param   {Function} mergeStrategy A function that can resolve a
 *                                   merge issues
 *
 * @public
 * @returns {Mixed} value
...
Record.prototype.setMergeStrategy = function( mergeStrategy ) {
	if( mergeStrategy instanceof Function ) {
		...
		this.emit( 'error', 'MergeStrategy requires to be a Function' );
	}
};
...
Record.prototype._applyUpdate = function( message ) {
	...
	if( this.version === null ) {
		...
		*/
		if( message.action === C.ACTIONS.PATCH ) {
			var dataTemp = utils.deepCopy( this._$data );
			this._getPath( message.data[ 2 ] ).setValue( data );
			data = utils.deepCopy( this._$data );
			this._$data = dataTemp;
		}
