...
import { Topics, Events, ConnectionStates, Actions } from "./constants/Constants";
...
export class Client extends Emitter {
    ...
    private _messageCallbacks: {[key: string]: Function};

    public get options(): DeepstreamOptions {
    ...
    public constructor(url: string, options?: DeepstreamUserOptions) {
        ...
        this._connection = new Connection(this, this._url);

        this.event = new EventHandler(this, this._connection);
        this.rpc = new RPCHandler(this, this._connection);
        ...
        this._messageCallbacks = {
            [Topics.EVENT]: this.event._$handle.bind(this.event),
            [Topics.RPC]: this.rpc._$handle.bind(this.rpc),
            [Topics.RECORD]: this.record._$handle.bind(this.record),
            [Topics.PRESENCE]: this.presence._$handle.bind(this.presence),
            [Topics.ERROR]: this._onErrorMessage.bind(this)
        };
    }
    ...
    public _$onMessage(message: ParsedMessage): void {
        ...
        if (message.action === Actions.ERROR && !message.processedError) {
            this._$onError(message.topic, message.data[0], message.data[1]);
        }
    }
     ...
     * @param   {String} message   Error dependent message
     *
     * @package private
     * @returns {void}
     */
    public _$onError(topic: string, event: string, message: string): void {
        ...
        if (event === Events.ACK_TIMEOUT || event === Events.RESPONSE_TIMEOUT) {
            if (this.connectionState === ConnectionStates.AWAITING_AUTHENTICATION) {
        ...
        if (this.hasListeners('error')) {
            this.emit('error', message, event, topic);
     ...
     * @returns {void}
     */
    private _onErrorMessage(errorMessage: ParsedMessage): void {
    ...
    private _mergeWithDefaultOptions(options: DeepstreamUserOptions): DeepstreamOptions {
        let mergedOptions: DeepstreamOptions = {} as any;

        for (let key in DefaultOptions) {
            if (typeof (options as any)[key] === 'undefined') {
                (mergedOptions as any)[key] = (DefaultOptions as any)[key];
            } else {
